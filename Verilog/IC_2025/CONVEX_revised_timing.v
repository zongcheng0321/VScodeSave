...
// 共線無法用外積去解決，後來想到用兩點距離判斷解決
// 直線方程式並不能判斷點是否在某點的左邊或右邊或順時鐘或逆時鐘，所以直線方程式完全用不到

// 修改完後 P2 一樣過不了，但是面積減少很多到了 70000，compile -map_effort high - area_effort high timing 也通過了
// 甚至修改了SDC -> set_clock_uncertainty -setup 0.1 [all_clocks] -> set_clock_uncertainty -setup 0.5 [all_clocks] 面積變大timing 變成7.62 但還是過不了!
// 最後發現重製所有 datapath及 controlpath 的暫存器，就通過了，但到底是哪個暫存器在搞還有待商榷

// compile -map_effort high -area_effort high timing - 0.92
/*
 ## 兇手現形：一條長達 9.16ns 的「死亡路徑」
 
 我們來讀一下報告中的 Critical Path（關鍵路徑）：
 
 Startpoint (起點)：point_cnt_reg[1]
 
 Endpoint (終點)：cnt_reg[1]
 
 為什麼從 point_cnt 走到 cnt 會花掉整整 9.16ns？我們把報告對應回你的 RTL 程式碼：
 
 讀取 point_cnt 暫存器。
 
 計算 point_num_minus_1 (減法器)。
 
 判斷 NEXT_POINT_POSITION (比較器 + MUX)。
 
 利用這個指標去陣列抓座標 point_sorted_x[NEXT_POINT_POSITION] (超大 MUX)。
 
 計算 nextP_minus_currP_x (減法器 sub_91)。
 
 進入 11-bit 乘法器 (mult_129) 算出 $X^2$ 與 $Y^2$。
 
 進入 22-bit 加法器 (add_66) 算出 distance_temp。
 
 進入 比較器 (r871) 判斷距離大小 (cmp_distance_greater)。
 
 透過條件判斷，決定下一個狀態的 cnt 該更新為多少，並存入 cnt_reg。
*/
...
// ***修改處***
// 共線計算專用的暫存器:為了減少 2.77ns 的路徑延遲
reg [9:0] line_p0_x, line_p0_y;
reg [9:0] line_p1_x, line_p1_y;
...
// ***修改處***
assign nextP_minus_currP_x = $signed({1'd0, line_p1_x}) - $signed({1'd0, line_p0_x});
assign nextP_minus_currP_y = $signed({1'd0, line_p1_y}) - $signed({1'd0, line_p0_y});
// ***********
// 0 -> new dis
wire signed [10:0] new_minus_currP_x;
wire signed [10:0] new_minus_currP_y;
// ***修改處***
assign new_minus_currP_x = $signed({1'd0, new_x}) - $signed({1'd0, line_p0_x});
assign new_minus_currP_y = $signed({1'd0, new_y}) - $signed({1'd0, line_p0_y});
// ***********
// 1 -> new dis
wire signed [10:0] new_minus_nextP_x;
wire signed [10:0] new_minus_nextP_y;
// ***修改處***
assign new_minus_nextP_x = $signed({1'd0, new_x}) - $signed({1'd0, line_p1_x});
assign new_minus_nextP_y = $signed({1'd0, new_y}) - $signed({1'd0, line_p1_y});
// ***********
...
always @(posedge CLK or posedge RST) begin
    if (RST) begin
        ...
        // datapath 跟 control path 全部重製面積會比較大，只要 rst control path 就好，但有 post sim 的問題(don't care 蔓延)，尚未找出是哪個 reg 在搞鬼 (future)
        // --- 輸出訊號 ---
        READ_PT <= 1'd1;
        DROP_X  <= 10'd0;
        DROP_Y  <= 10'd0;
        ...
        cnt <= 4'd0;
        point_cnt <= 4'd0;
        tangent_point_cnt <= 2'd0;
        have_tangentORconcave_point <= 1'd0;
        is_write_insert_new_point_position <= 1'd0;
        DROP_point_yield <= 1'd0;
        DROP_last_point <= 1'd0;
        resort_cnt <= 4'd0;
        is_output_next_point <= 1'd0;
        is_same_line <= 1'd0;
        first_3p_sort <= 1'd0;
        insert_new_point_position <= 4'd0;
        tangent_point_equal_first_point <= 1'd0;
        CW_OR_CCW_adjacent <= 1'd0;
        is_same_side_1 <= 1'd0;
        p_01_smaller_p_0new <= 1'd0;
        p_01_greater_p_0new <= 1'd0;
        // --- 座標與運算暫存器 ---
        new_x <= 10'd0; new_y <= 10'd0;
        line_p0_x <= 10'd0; line_p0_y <= 10'd0;
        line_p1_x <= 10'd0; line_p1_y <= 10'd0;
        ox <= 10'd0; oy <= 10'd0;
        ax2 <= 10'd0; ay2 <= 10'd0; 
        bx2 <= 10'd0; by2 <= 10'd0;
        distance_temp1 <= 0;
        ...
        for (i = 0; i < 12; i = i + 1) begin
            point_sorted_x[i] <= 10'd0;
            point_sorted_y[i] <= 10'd0;
        end
    end else begin
        case (state)
            INPUT: begin
            ...
            DROP2: begin
                ...
                if (cross_output == 0) begin // 共線只會被 Ax 和 新點 做外積時發現
                    is_same_line <= 1'd1;
                    state <= SAME_LINE;
                    // ***修改處*** 先在前一個 clk 把資料先準備好
                    line_p0_x <= point_sorted_x[cnt];
                    line_p0_y <= point_sorted_y[cnt];
                    line_p1_x <= point_sorted_x[NEXT_POINT_POSITION];
                    line_p1_y <= point_sorted_y[NEXT_POINT_POSITION];
                end else begin
                    is_same_line <= 0;
                end
            end
