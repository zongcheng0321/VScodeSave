...
//------------------------------------------------------------------------------------------------------
// Timing Report
// 抓頭尾找關聯： 每次看 Timing Report，先看 Startpoint 和 Endpoint。去 code 裡面 Ctrl+F 搜尋這兩個變數，找它們在哪裡被放在同一個 if 條件裡，或者是誰變成了誰的 index。
// 留意隱形的硬體怪物： 陣列的變數索引 (array[Variable])、長串的 if-else、大範圍的 + - > < 運算，都是非常吃路徑延遲（Delay）的結構。
// 善用 EDA GUI 工具： 在 Design Compiler 中，你可以使用 Design Vision（圖形化介面），選擇這條 Timing Path 並要求它畫出 Schematic（電路圖）。
// 這樣你就能親眼看到暫存器出來後接了哪些 MUX 和比較器，會比單看程式碼和文字報告更有感覺。
//------------------------------------------------------------------------------------------------------
// 要把S2、3、4改成不要用迴圈的方式
reg [2:0] array [7:0]; 
