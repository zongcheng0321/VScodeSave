 ...
 */
    reg [2:0] stage1_array [0:7];
    reg [2:0] next_array [0:7];

    always @(*) begin
        // ==========================================
        // 第一階段：執行 Swap (交換 cp 和 sp)
        // ==========================================
        stage1_array[0] = array[0];
        stage1_array[1] = array[1];
        stage1_array[2] = array[2];
        stage1_array[3] = array[3];
        stage1_array[4] = array[4];
        stage1_array[5] = array[5];
        stage1_array[6] = array[6];
        stage1_array[7] = array[7];
        
        stage1_array[cp] = array[sp];
        stage1_array[sp] = array[cp];


        // ==========================================
        // 第二階段：執行 Reverse (根據 cp 翻轉尾部)
        // ==========================================
        // 先預設下一代等於第一階段(Swap完)的結果
        next_array[0] = stage1_array[0];
        next_array[1] = stage1_array[1];
        next_array[2] = stage1_array[2];
        next_array[3] = stage1_array[3];
        next_array[4] = stage1_array[4];
        next_array[5] = stage1_array[5];
        next_array[6] = stage1_array[6];
        next_array[7] = stage1_array[7];

        // 再根據 cp 覆蓋需要翻轉的部分
        case (cp)
            3'd0: begin
                next_array[1] = stage1_array[7]; next_array[7] = stage1_array[1];
                next_array[2] = stage1_array[6]; next_array[6] = stage1_array[2];
                next_array[3] = stage1_array[5]; next_array[5] = stage1_array[3];
            end
            3'd1: begin
                next_array[2] = stage1_array[7]; next_array[7] = stage1_array[2];
                next_array[3] = stage1_array[6]; next_array[6] = stage1_array[3];
                next_array[4] = stage1_array[5]; next_array[5] = stage1_array[4];
            end
            3'd2: begin
                next_array[3] = stage1_array[7]; next_array[7] = stage1_array[3];
                next_array[4] = stage1_array[6]; next_array[6] = stage1_array[4];
            end
            3'd3: begin
                next_array[4] = stage1_array[7]; next_array[7] = stage1_array[4];
                next_array[5] = stage1_array[6]; next_array[6] = stage1_array[5];
            end
            3'd4: begin
                next_array[5] = stage1_array[7]; next_array[7] = stage1_array[5];
            end
            3'd5: begin
                next_array[6] = stage1_array[7]; next_array[7] = stage1_array[6];
            end
            default: begin
                // cp 為 6 或 7 時，沒有足夠的元素可以翻轉，維持 stage1_array 的狀態即可
            end
        endcase
    end
