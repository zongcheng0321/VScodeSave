...
// 原本沒有 pipeline area 160270 timing -5.6 
...
// 先奇偶排序 -> 排序顏色 -> 輸出
// 奇偶排序 時間為 5244830ns
// 此電路為改善 critical path，根據 report timing 推測應該為除法器路徑太長 -> 看能不能改善面積及速度到達 10 NS 以內

// 改變 num_stages = 2 -> slack -0.06 area 162515
//                   3 -> slack +0.01 area 130609, gatelevel 5245470 NS
//                   4 -> slack +0.00 area 117660, gatelevel 5245790 NS
//                   5 -> slack +0.00 area 114885
//      **BEST       10 -> slack +0.00 area 94217, gatelevel 5247710 NS
//                   20 -> slacl +0.00 area 97962
// 不管怎麼增加 num_stages 都沒辦法把 slack 縮至 10 ns 以內
//------------------------------------------
`timescale 1ns/10ps
...
DW_div_pipe #(.a_width(a_width), 
        ...
        /*(操作隔離)：0 代表關閉，1 代表開啟。這是用來做低功耗設計的，把沒在用的訊號線切斷防漏電，關閉以節省面積*/
        .op_iso_mode(0)) 
        U1 (
        .clk(clk),            
        .rst_n(~reset),         
    ...
    if (reset) begin
        ...
        case (state)
            ...
            AVG_INTENSITY: begin
                ...
                Avg_intensity[curr_image] <= quotient[FRAC_BIT + 8 - 1 : 0]; // pipeline 10 stages
                busy <= 0; // 結束暫停
            ...
            // 先排強度 (奇偶)
            SORT_INTENSITY: begin // 共 31 個比較器 (even:16 odd:15)
                if (cnt == 5'd31) begin
                    cnt <= 0;
                    state <= OUTPUT_COLOR; 
                end else begin
                ...
                if (cnt[0] == 1'b0) begin 
                    ...
                    for (i = 0; i < 32; i = i + 2) begin
                        if (Avg_intensity[i] > Avg_intensity[i+1]) begin
                            Avg_intensity[i] <= Avg_intensity[i+1]; Avg_intensity[i+1] <= Avg_intensity[i];
                            image_num[i]     <= image_num[i+1];     image_num[i+1]     <= image_num[i];
                            color_index_store[i] <= color_index_store[i+1]; color_index_store[i+1] <= color_index_store[i];
                        end
                    end
                // 當 cnt 為奇數
                end else begin
                    // 奇數(沒有對第 0 個跟第 31 個位置做比較，所以少 1 比較器)
