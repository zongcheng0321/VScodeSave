...
// 做完 SORT_COLOR 去跑合成(compile 面積effort map effort) timing +0.94 area 11618 (可能沒跑到除法器，被拔掉了)
...
//------------------------------------------
// 這邊先做奇偶排序，這邊為了方便就不做提前結束排序的功能(當沒有任何人交換)
// 先排強度，後排顏色

// 可能還需要對除法器做 pipeline 不然 timing 過不了面積又很大
// 做完之後發現我的原本顏色排序法雖然可以排完，但會把排好的強度打亂
// 所以必須根據已排好的強度的陣列，想一個新的顏色排序法
//------------------------------------------
`timescale 1ns/10ps
...
always @(posedge clk or posedge reset) begin
    if (reset) begin
        ...
        case (state)
            ...
            // 顏色可能會從缺
            SORT_COLOR: begin
                // 如果 [cnt] 跟 [pointer] 是同個顏色的話，cnt 就不加，pointer--
                // 不是同一個顏色 或 是紅色 -> cnt ++
                if ((color_index_store[cnt] != color_index_store[pointer]) || color_index_store[cnt] == 2'b00) begin 
                    cnt <= cnt + 1'd1;
                end

                // 狀態轉移條件
                if (cnt >= pointer) begin // 當 cnt 跟 pointer 交叉 -> 結束
                    cnt <= 0;
                    is_sort_green <= 1'd1; 
            ...
            OUTPUT: begin
                cnt <= cnt + 1'd1;
                out_valid <= 1'd1;
                color_index <= color_index_store[cnt];
